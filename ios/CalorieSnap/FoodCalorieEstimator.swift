    ...
    func estimateCalories(for image: UIImage, completion: @escaping (Result<FoodCalorieEstimate, Error>) -> Void) {
        queue.async {
            ...
            do {
                ...
                guard
                    let observations = self.request.results as? [VNClassificationObservation],
                ...
                let calorieValue = self.calorieLookup.first { normalizedIdentifier.contains($0.key) }?.value
                    ?? self.heuristicCalories(forConfidence: best.confidence)
